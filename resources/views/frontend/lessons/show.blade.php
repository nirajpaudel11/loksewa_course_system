...
<div class="flex flex-col lg:flex-row gap-8 max-w-7xl mx-auto">
    ...
    <div class="flex-1 min-w-0">
        ...
        @if($lesson->type == 'video' || !empty($lesson->video_url))
            @php
                $videoSrc = null;
                if (!empty($lesson->attachment_path)) {
                    if (\Illuminate\Support\Str::startsWith($lesson->attachment_path, ['http://', 'https://'])) {
            ...
            <div class="bg-gray-950 rounded-3xl shadow-2xl mb-8 relative border border-gray-800 overflow-hidden">
                @if(!empty($lesson->video_url))
                    <div class="aspect-video w-full">
                        <iframe src="{{ $lesson->video_url }}" class="w-full h-full border-0 rounded-3xl" allowfullscreen allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"></iframe>
                    </div>
                @elseif($videoSrc)
            ...
            <div class="bg-gray-900 border border-gray-800 rounded-3xl overflow-hidden shadow-2xl mb-8">
                ...
                <div class="bg-gray-950/90 backdrop-blur border-b border-gray-800 px-6 py-4 flex flex-wrap items-center justify-between gap-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h4 class="text-sm font-bold text-gray-100 truncate max-w-xs sm:max-w-md">{{ $lesson->title }} — Study Document</h4>
                            <span class="text-[10px] text-gray-400 font-medium">Official Printable PDF &bull; Loksewa Syllabus Reference</span>
                        </div>
                    </div>
                    <div class="flex items-center gap-2">
                        <a href="{{ $pdfUrl }}" target="_blank" rel="noopener noreferrer" class="px-3.5 py-1.5 rounded-xl bg-gray-800 hover:bg-gray-700 text-gray-200 text-xs font-semibold flex items-center gap-1.5 transition-colors border border-gray-700 shadow-sm" title="Open PDF in Full Tab">
                            <i data-lucide="external-link" class="w-3.5 h-3.5 text-emerald-400"></i>
                            <span>Open Tab</span>
                        </a>
                        <a href="{{ $pdfUrl }}" download class="px-4 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold flex items-center gap-1.5 transition-colors shadow-sm">
                ...
                <div class="bg-gray-950 px-6 py-3 border-t border-gray-800/80 flex flex-wrap items-center justify-between text-xs text-gray-400 gap-2">
                    <div class="flex items-center gap-2">
                        ...
                        <span>Dedicated Lesson Material for {{ $lesson->title }}</span>
                    </div>
                    <div class="flex items-center gap-3">
                        <span class="text-[11px] text-gray-500">Trouble viewing?</span>
                        <a href="{{ $pdfUrl }}" target="_blank" class="text-emerald-400 hover:underline font-semibold">Open directly in browser</a>
                    </div>
                </div>
            </div>
        @endif
        ...
        @if($quizData && count($quizData) > 0)
            <div class="bg-gray-950 rounded-3xl shadow-2xl mb-8 border border-gray-800 overflow-hidden">
                <div id="quiz-container" class="text-white select-none p-6 md:p-8" data-questions="{{ json_encode($quizData) }}">
                    ...
                    <div class="flex items-center justify-between border-b border-gray-800 pb-4 mb-4">
                        <div>
                            ...
                            <p class="text-sm font-bold text-gray-200">{{ $lesson->title }} — Practice Test</p>
                        </div>
                        <div class="text-right">
                            <p id="quiz-progress-text" class="text-xs font-bold text-gray-400">Question 1 of {{ count($quizData) }}</p>
                ...
                <div id="quiz-results-container" class="hidden p-8 text-white text-center">
                    ...
                    <form action="{{ route('lessons.complete', [$course->slug, $lesson->slug]) }}" method="POST" class="max-w-xs mx-auto">
                        ...
                        <input type="hidden" name="quiz_total" id="quiz-final-total-input" value="{{ count($quizData ?? []) }}">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 pb-6 border-b border-gray-100">
            <div>
                ...
                <div class="flex flex-wrap items-center gap-3">
                    ...
                    @if(isset($predictedCompletion) && $predictedCompletion)
                        <span class="inline-flex items-center gap-1.5 px-3 py-1 bg-emerald-50 border border-emerald-200 text-emerald-800 text-[11px] font-bold rounded-full">
                            ...
                            Est. Course Finish: {{ $predictedCompletion->format('M d, Y') }} ({{ number_format($paceMultiplier ?? 1.0, 2) }}× pace)
                        </span>
                    @endif
                </div>
            </div>
            ...
            @else
                <div class="flex items-center gap-2">
                    <span class="inline-flex items-center gap-2 px-4 py-2.5 rounded-2xl bg-amber-50 border border-amber-200 text-amber-800 font-bold text-xs shadow-sm">
                        ...
                        <span>Quiz Practice Required ({{ count($quizData) }} MCQs)</span>
                    </span>
                </div>
            @endif
        </div>
        ...
        <div class="mb-10">
            ...
            <div id="tab-content" class="tab-panel prose prose-emerald max-w-none text-gray-600 leading-relaxed bg-white p-8 rounded-3xl border border-gray-100 shadow-sm">
                ...
                @endphp
                @if(!$isJsonContent && !empty($lesson->content))
                    {!! $lesson->content !!}
                @elseif($quizData && count($quizData) > 0)
                ...
                @else
                    <div class="not-prose text-center py-6 text-gray-400">
                        ...
                        <p class="text-sm font-semibold text-gray-600">Overview Notes for {{ $lesson->title }}</p>
            ...
            <div id="tab-notes" class="tab-panel hidden bg-white p-8 rounded-3xl border border-gray-100 shadow-sm leading-relaxed text-gray-600">
                @if(!empty($lesson->module?->notes) || !empty($lesson->module?->key_points))
                    <h4 class="text-lg font-bold text-gray-900 mb-4">Syllabus Key Focus & Study Notes</h4>
                    
                    @if(!empty($lesson->module?->key_points))
                        <div class="mb-6 bg-amber-50/80 border border-amber-200/80 rounded-2xl p-5 text-sm text-amber-900 leading-relaxed font-medium">
                            ...
                            {!! nl2br(e($lesson->module->key_points)) !!}
                        </div>
                    @endif

                    @if(!empty($lesson->module?->notes))
                        <div class="prose prose-emerald max-w-none text-gray-700 leading-relaxed">
                            {!! nl2br(e($lesson->module->notes)) !!}
                        </div>
                    @endif
                @else
                ...
                @foreach($course->modules->sortBy('order') as $module)
                    <div class="mb-4">
                        <div class="px-4 py-2 flex items-center justify-between">
                            <span class="text-[10px] font-black text-emerald-600 uppercase tracking-widest">{{ $module->title }}</span>
                            <span class="text-[9px] font-bold px-2 py-0.5 rounded-full bg-gray-100 text-gray-500">M{{ $module->order ?? $loop->iteration }}</span>
                        </div>
                        @php
                        ...
                        @foreach($moduleLessons as $l)
                            ...
                            @endphp
                            @if($isUnlocked)
                                <a href="{{ route('lessons.show', [$course->slug, $l->slug]) }}" class="flex items-center justify-between px-4 py-3 rounded-2xl transition-all {{ $isCurrent ? 'bg-emerald-50 text-emerald-700 font-bold border border-emerald-200/60 shadow-sm' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }}">
                                    <span class="flex items-center gap-2.5 truncate">
                                        <span class="w-5 h-5 rounded-lg {{ $isCurrent ? 'bg-emerald-600 text-white' : ($isCompleted ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-500') }} text-[10px] font-black flex items-center justify-center shrink-0">
                                            {{ $l->order ?? $loop->iteration }}
                                        </span>
                                        <i data-lucide="book-open" class="w-3.5 h-3.5 {{ $isCurrent ? 'text-emerald-600' : ($isCompleted ? 'text-emerald-500' : 'text-gray-400') }} shrink-0"></i>
                                        <span class="text-xs truncate font-medium">{{ $l->title }}</span>
                                    </span>
                                    @if($isCompleted)
                            ...
                            @else
                                <div class="flex items-center justify-between px-4 py-3 rounded-2xl text-gray-400 bg-gray-50/50 cursor-not-allowed opacity-75 select-none" title="Complete previous lessons to unlock">
                                    <span class="flex items-center gap-2.5 truncate">
                                        ...
                                        <span class="text-xs truncate font-normal text-gray-400">{{ $l->title }}</span>
                                    </span>
