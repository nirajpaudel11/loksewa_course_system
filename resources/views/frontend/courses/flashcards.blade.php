...
@section('title', ($course ? $course->title . ' - ' : '') . 'Spaced Repetition Flashcard Review')
    ...
    <div class="max-w-4xl mx-auto relative z-10">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 pb-4 border-b border-slate-800/80">
            <div class="flex items-center gap-3">
                <a href="{{ $course ? route('courses.details', $course->slug) : route('dashboard') }}" 
                   class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-900 border border-slate-700/80 hover:border-slate-600 text-slate-300 hover:text-white text-xs font-semibold transition-all shadow-sm">
                    ...
                    <span>{{ $course ? 'Back to Course' : 'Back to Dashboard' }}</span>
                </a>
                <div>
                    ...
                    <h1 class="text-lg md:text-xl font-black text-white tracking-tight mt-0.5">
                        {{ $course ? $course->title : 'Due Spaced Repetition Reviews' }}
                    </h1>
                </div>
            </div>
            ...
            @if($course)
                <div class="flex items-center gap-2 bg-slate-900/90 p-1 rounded-xl border border-slate-800">
                    <a href="{{ route('courses.flashcards', [$course->slug, 'mode' => 'due']) }}" 
                       class="px-3 py-1 text-xs font-bold rounded-lg transition-all {{ $mode === 'due' ? 'bg-amber-500 text-slate-950 shadow-md' : 'text-slate-400 hover:text-white' }}">
                        Due Only ({{ $dueCount }})
                    </a>
                    <a href="{{ route('courses.flashcards', [$course->slug, 'mode' => 'all']) }}" 
                       class="px-3 py-1 text-xs font-bold rounded-lg transition-all {{ $mode === 'all' ? 'bg-amber-500 text-slate-950 shadow-md' : 'text-slate-400 hover:text-white' }}">
            ...
            <div class="bg-slate-900/90 border border-slate-800 rounded-3xl p-10 text-center shadow-2xl backdrop-blur-xl">
                ...
                @if($course)
                    <div class="flex items-center justify-center gap-3">
                        <a href="{{ route('courses.flashcards', [$course->slug, 'mode' => 'all']) }}" class="px-5 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-600 text-slate-950 font-bold text-xs transition-all shadow-md">
                            Review All {{ $totalCount }} Lessons Anyway
                        </a>
                        <a href="{{ route('dashboard') }}" class="px-5 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 font-bold text-xs transition-all border border-slate-700">
                            Return to Dashboard
                        </a>
                    </div>
                @else
                    <a href="{{ route('dashboard') }}" class="px-5 py-2.5 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-slate-950 font-bold text-xs transition-all shadow-md">
                        Return to Dashboard
                    </a>
                @endif
            </div>
        @else
            ...
            <div id="flashcardApp" class="space-y-6">
                ...
                <div class="flex items-center justify-between text-xs font-bold text-slate-400 px-1">
                    <span id="cardProgressText">Card 1 of {{ $flashcards->count() }}</span>
            ...
            <div id="completionSummary" class="hidden bg-slate-900/95 border border-emerald-500/40 rounded-3xl p-8 md:p-10 text-center shadow-2xl backdrop-blur-xl">
                ...
                <div class="flex flex-wrap items-center justify-center gap-3">
                    ...
                    @if($course)
                        <a href="{{ route('courses.details', $course->slug) }}" class="px-5 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-600 text-slate-950 font-black text-xs transition-all shadow-md">
                            Continue Course Curriculum
                        </a>
                    @endif
                    <a href="{{ route('dashboard') }}" class="px-5 py-2.5 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-slate-950 font-black text-xs transition-all shadow-md">
                        Return to Dashboard
                    </a>
                </div>
            </div>
