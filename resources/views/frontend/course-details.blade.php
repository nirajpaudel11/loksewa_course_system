...
<div class="max-w-7xl mx-auto">
    ...
    @if($isEnrolled && $enrollment && $enrollment->status === 'rejected')
        <div class="mb-8 p-4 bg-rose-50 border border-rose-200 text-rose-900 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-sm">
            ...
            <form action="{{ route('courses.enroll', $course->id) }}" method="POST" class="shrink-0 w-full sm:w-auto">
                @csrf
                    ...
                    @forelse($course->modules as $index => $mod)
                        ...
                        <div class="bg-white rounded-3xl border border-gray-100 p-6 shadow-sm hover:shadow-md hover:border-emerald-100 transition-all flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                            ...
                            <div class="shrink-0 w-full md:w-auto">
                                ...
                                @elseif($isEnrolled && $enrollment && $enrollment->status === 'rejected')
                        ...
                        <div class="space-y-4">
                            ...
                            @if($targetUrl)
                                <a href="{{ $targetUrl }}" class="w-full block text-center bg-gray-900 hover:bg-black text-white py-4 rounded-2xl font-bold text-base transition-all shadow-xl">
                                    {{ $isCourseCompleted ? 'Review Course Material 📖' : (($progressPercentage ?? 0) > 0 ? 'Resume Learning 🚀' : 'Start Learning 🚀') }}
                                </a>
                            @else
