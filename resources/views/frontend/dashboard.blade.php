...
@if(isset($rejectedEnrollments) && $rejectedEnrollments->isNotEmpty())
    <div class="mb-6 space-y-3">
        @foreach($rejectedEnrollments as $rej)
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs font-medium text-rose-900 mt-0.5">
                            Your enrollment request for <strong>{{ $rej->course->title ?? 'a course' }}</strong> was rejected by the administrator.
                        </p>
                    </div>
                </div>
                @if($rej->course)
                    <a href="{{ url('/courses/' . $rej->course->slug) }}" class="px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold rounded-xl transition-all shadow-sm shrink-0 flex items-center gap-1.5">
                ...
                <div class="bg-white rounded-3xl border border-gray-100 p-6 shadow-sm hover:shadow-md transition-all flex flex-col justify-between">
                    <div>
                        ...
                        <div class="flex items-start justify-between gap-3 mb-4">
                            ...
                            @elseif(($item['enrollment']->status ?? 'active') === 'rejected')
                        ...
                        @if(($item['enrollment']->status ?? 'active') === 'rejected')
                    ...
                    <div class="pt-2 border-t border-gray-50">
                        @if(($item['enrollment']->status ?? 'active') === 'rejected')
                            <a href="{{ url('/courses/' . $item['course']->slug) }}" class="w-full inline-flex items-center justify-center gap-2 py-3 px-4 rounded-2xl bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs transition-all shadow-sm">
                                <span>Re-apply for Enrollment</span>
                                <i data-lucide="refresh-cw" class="w-3.5 h-3.5"></i>
                            </a>
                        @elseif(($item['enrollment']->status ?? 'active') === 'pending')
